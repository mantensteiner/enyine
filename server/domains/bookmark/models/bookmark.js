var BaseModel = require('../../_shared/baseModel'),
    ...
    modelDefinition = require('../config/mapping').bookmark.properties,
  ...
  this.new = function() {
    ...
    model.validate = function(modelData) {
      if(!modelData.spaceId && !modelData.userId) 
        return q.fcall(function () { throw new ValidationError('SpaceId or UserId missing.'); });
    ...
    model.validateSave = function(modelData) {
      ...
      .fail(function(err) {
        log.error(err, {name: 'bookmark.validateSave', modelData: modelData});
